...
fn generate_skill_levels() -> Vec<Player> {
    ...
    // Iteratively purge all players that have never won or never lost.
    loop {
        let mut indices = Vec::<usize>::new(); // This will be ordered from smallest to largest.

        for (index, player) in players.iter().enumerate() {
            let (mut total_wins, mut total_losses) = (0., 0.);
        ...
        if indices.is_empty() {
            // No more players to purge; kill the loop.
            break;
        }
    ...
    for _ in 0..MAX_ITERATIONS {
        ...
        let mut augmented_hessian = vec![vec![1f64; n + 2]; n + 1]; // The hessian matrix augmented with the negative of the gradient matrix.
