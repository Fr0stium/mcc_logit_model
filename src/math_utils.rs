...
/// Computes the hazard rate of the standard normal distribution.
pub fn hazard(x: f64) -> f64 {
    if x >= 20.0 {
        // Very good approximation for x > 20:
        return x + 1.0 / x - 2.0 * x.powi(-3) + 10.0 * x.powi(-5) - 74.0 * x.powi(-7);
    }
    standard_normal_pdf(x) / (standard_normal_cdf(-x))
}

/// Computes the derivative of the hazard rate of the standard normal distribution.
